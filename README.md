...
## 共通規範は user memory へ集約 / template は薄い骨格 (CLAUDE.md ダイエット, [#90](https://github.com/ippoan/claude-md/issues/90))

org 全 repo 共通の作業規範 (issue→PR→push / 決めつけで動かない / lib-first /
branch・worktree / GitHub 自動化 (auto-merge enable 禁止・`Refs #N` 等) / secret /
やってはいけないこと) は **`.claude/user-memory.md`** に集約し、install.sh が
`~/.claude/CLAUDE.md` (user memory) に毎 session 配布する (下記「User memory」節)。

理由: CCoW は attach された repo が可変で、しかも各 repo の `CLAUDE.md` を**丸ごと
eager 注入**する。共通規範を各 repo の CLAUDE.md に重複させると (a) その規範が N 重に
基底 context を膨らませ、sonnet サブエージェントが context 過多で落ちる、(b) claude-md が
attach されない session では消える、の両問題が出る。user memory に置けば「どの repo が
attach されても消えない fallback」かつ「基底を 1 コピーに抑える」を両立できる。
repo 固有の例外は project memory (repo の `CLAUDE.md`) が user memory を override する。

`CLAUDE.md.template` は共通規範を**繰り返さない薄い骨格**にとどめる:

| # | セクション | どう書くか |
|---|---|---|
| 1 | 冒頭タイトル + 1 行 intro | `# CLAUDE.md` + repo の存在意義を 1 文 |
| 2 | **まず読むもの** | README + `<repo>-map` skill への**条件付きポインタ**（詳細は map skill に退避、lazy 読み込み） |
| 3 | **この repo 固有の invariant** | 共通規範で足りない短い禁止・必須 (1 行 invariant、理由・経緯は map skill) |
| 4 | **ビルド / テスト / lint** | PR 前に叩く command (CI と同じ) |
| 5 | **CI / auto-merge (repo 固有設定)** | Required status checks 等の repo 固有値のみ (共通ルールは user memory) |
| 6 | repo 固有ドメインセクション | 末尾 `<<DOMAIN_SPECIFIC_SECTIONS>>` |

repo 固有の詳細 (テーブル定義・エンドポイント表・gotcha 集・デプロイ手順) は CLAUDE.md に
直接書かず **`<repo>-map` skill** (ippoan/claude-skills、lazy) に退避する。CLAUDE.md からは
「〜を触る前に `<repo>-map` §X 必読」の条件付きポインタで誘導する。

## Variants

repo 種別に応じて template の §4 (ビルド/テスト/lint) と §5 (CI/auto-merge の repo 固有設定) だけ差し替えると 90% カバーできる:
...
| 2. **user-memory CLAUDE.md** | `.claude/user-memory.md` を取得して `~/.claude/CLAUDE.md` に marker block (`<!-- BEGIN claude-md user-memory:locale -->` … `<!-- END … -->`) として idempotent に merge。block 外の手書き内容は保持。応答言語を日本語 default にする ([#26](https://github.com/ippoan/claude-md/issues/26)) + org 共通作業規範を always-on 配布する ([#90](https://github.com/ippoan/claude-md/issues/90)) | `SKIP_USER_MEMORY=1` |
...
2 つの役割を担う (CLAUDE.md ダイエット [#90](https://github.com/ippoan/claude-md/issues/90)):

1. **応答言語 default (日本語)** — Claude Code の `settings.json` schema には `locale` / `language` に相当する key が存在しない ([#26](https://github.com/ippoan/claude-md/issues/26)) ため user memory 経由で注入する。settings.json 側の `locale: "ja"` は forward-compatible な hint で現状 ignore される。
2. **org 共通作業規範 (always-on fallback)** — issue→PR→push / branch / GitHub 自動化 / secret / やってはいけないこと 等の org 全 repo 共通ルール。CCoW は attach された repo が可変なので、共通規範を user memory (どの session でも install.sh が必ず配布) に置くことで「claude-md が attach されない session でも消えない」かつ「各 repo の CLAUDE.md に重複させず基底 context を膨らませない」を両立する。repo 固有の override は project memory (repo の `CLAUDE.md`) が user memory に勝つ。

`user-memory.md` を編集すると次 session から全 environment に伝播する (install.sh の sha が変わり refresh-installer が再配布)。共通規範を増減したい時はここ (と `CLAUDE.md.template`) を直す。
...
| `CLAUDE_SKILL_COVERAGE_IGNORE` | `claude-skills` | hook 側 (`session-start-skill-coverage.sh`): coverage / 鮮度チェックから除外する repo (space 区切り)。`claude-skills` は `<repo>-map` の置き場そのもの (= 追従すべき外部 code を持たない) で、自分自身を map した `claude-skills-map` は repo 変更の度に無意味に stale 化するため既定で除外する |
