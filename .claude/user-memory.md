...
**Note:** auto-merge / `Refs #N` / branch protection 等の repo-policy は
各 repo の `CLAUDE.md` (project memory; `CLAUDE.md.template` から派生)
に書く。user memory には書かない (project memory が user memory より優先
されるため、user memory に書くと repo ごとの上書きが効かない上、
template との二重管理になる)。
